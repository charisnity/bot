...
export async function handler(chatUpdate) { 
    ...
    try {
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (m.chat in global.db.data.chats || m.sender in global.db.data.users) {
                  ...
                  if (name == 'group-hidetag.js' && chat?.lockHidetag) return
                    if (!/mute|unban|kick|exec/.test(name) && chat?.isBanned) return // Except this
                    if (name != 'owner-unban.js' && user?.banned) return
                }
     ...
     //  if (!opts['autoread']) await this.chatRead(m.chat, m.isGroup ? m.sender : undefined, m.id || m.key.id).catch(() => { })
        if (global.db?.data?.settings?.[this.user.jid]?.autoread) await this.readMessages([m.key]).catch(() => { })
    }
}
    ...
    let date = d.toLocaleDateString(locale, {
      ...
      year: 'numeric'
                    ...
                    for (let user of participants) {
                       // let pp = 'https://telegra.ph/file/222e8d8371d12b90f17ef.jpg'
                        let pp = 'https://telegra.ph/file/cdf3a97c835dbe8dfb6a9.jpg'

                        let ppgc = 'https://telegra.ph/file/0c48ae59478198e172106.jpg'
                       
                       // let _bio = this.fetchStatus(user)
                       // let pp = await this.profilePictureUrl(m.chat, 'image')
                        try {
                            ...
                            text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc ? String.fromCharCode(8206).repeat(4001) + groupMetadata.desc : '').replace('@time', time).replace('@members', groupMetadata.participants.length).replace('@number', user.split('@')[0]).replace('@day', week).replace('@date', date) :
                                (chat.sBye || this.bye || conn.bye || 'Bye, @user!')).replace('@user', await this.getName(user))
                              let scurl = `https://wa.me/${await this.getName(user)}`
                             
                             let jwel = '「 𝑾𝒆𝒍𝒄𝒐𝒎𝒆 𝑴𝒆𝒔𝒔𝒂𝒈𝒆 」'
                             let jlea = '「 𝑳𝒆𝒂𝒗𝒆 𝑴𝒆𝒔𝒔𝒂𝒈𝒆 」'

                          /*  let wel = API('rhobot', '/api/maker/welcome1', {
                                pp: pp,
                            ...
                            let lea = API('LeysCoder', '/api/leave', {
                                profile: pp,
                                name: await this.getName(user),
                                group: await this.getName(id),
                                apikey:'dappakntlll'
                            })
                               //  bege: 'https://telegra.ph/file/c538a6f5b0649a7861174.png',
                            let wel = API('beta', '/api/creator/welcome', {
                                pepeUser: pp,
                                nama: await this.getName(user),
                                bege: 'https://images.pexels.com/photos/963278/pexels-photo-963278.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2',
                                namaGb: await this.getName(id),
                                pepeGb: ppgc,
                                totalMem: groupMetadata.participants.length,
                                apikey:'BetaBotz'
                            })
                            let lea = API('beta', '/api/creator/goodbye', {
                                pepeUser: pp,
                                nama: await this.getName(user),
                                bege: 'https://images.pexels.com/photos/963278/pexels-photo-963278.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2',
                                namaGb: await this.getName(id),
                       ...
                       //     https://betabotz-api.herokuapp.com/api/creator/welcome?nama=BetaBotz&namaGb=Beta%20Bot&pepeGb=https://l.top4top.io/p_2271zprr10.jpg&totalMem=100000&pepeUser=https://i.ibb.co/7KD9BSq/20210811-103425.jpg&bege=https://l.top4top.io/p_2271zprr10.jpg&apikey=BetaBotz

// https://betabotz-api.herokuapp.com/api/creator/goodbye?nama=BetaBotz&namaGb=Beta%20Bot&pepeGb=https://l.top4top.io/p_2271zprr10.jpg&totalMem=100000&pepeUser=https://i.ibb.co/7KD9BSq/20210811-103425.jpg&bege=https://l.top4top.io/p_2271zprr10.jpg&apikey=BetaBotz

                          //  await this.sendButtonImg(id, action === 'add' ? wel : lea, text, wm, action === 'add' ? 'selamat datang' : 'sampai jumpa', action === 'add' ? 'oke' : 'oke') 
                          /* fake ig buriq
                            await this.sendMessage(id, {text: text, thumbnail: await( await this.getFile(pp)).data , contextInfo:{ mentionedJid: [user] , externalAdReply: { showAdAttribution: true, renderLargerThumbnail: true,
mediaType: 2,
mediaUrl: 'https://instagram.com/rhosad_',
title: '「 𝑾𝒆𝒍𝒄𝒐𝒎𝒆 𝑴𝒆𝒔𝒔𝒂𝒈𝒆 」',
body: await this.getName(id),
sourceUrl:'https://www.instagram.com/p/Cch2IoGFomX/?utm_source=ig_web_copy_link' , thumbnail: { url: pp}, thumbnailUrl: pp
  }
 }}) */
                            
                          await conn.sendMessage(id, {document: fs.readFileSync('./index.js'), mimetype: 'application/doc', fileLength: 1, pageCount: 1, fileName: `Hi ${await this.getName(user)}`, contextInfo: { externalAdReply: { sourceUrl: scurl,title: action === 'add' ? jwel : jlea,body: 'From ' + await this.getName(id), mediaUrl: scurl, mediaType: 1, showAdAttribution: true, renderLargerThumbnail: true, thumbnail: { url: pp}, thumbnailUrl: pp}}, caption: text, footer: wm, buttons: [ {buttonId: 'oke', buttonText: {displayText: action === 'add' ? 'selamat datang' : 'sampai jumpa'}, type: 1},{buttonId: '.menu', buttonText: {displayText: 'Menu'}, type: 1} ], headerType: 4}) 
                         //   conn.send2ButtonLoc(id, wel, text, wm, action === 'add' ? 'selamat datang' : 'sampai jumpa', action === 'add' ? 'oke' : 'oke', 'All Menu', '.menu') 
                        }
                    }
                }
        break
