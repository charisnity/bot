...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    let sock = Object.defineProperties(conn, {
        ...
        resize: {
            async value(buffer, width, high) {
	const _jimp = await jimp.read(buffer)
	const min = _jimp.getWidth()
	const max = _jimp.getHeight()
	const cropped = _jimp.crop(0, 0, min, max)
	return {
		img: await cropped.scaleToFit(width, high).getBufferAsync(jimp.MIME_JPEG),
		preview: await cropped.scaleToFit(width, high).getBufferAsync(jimp.MIME_JPEG)
	}
},
...
${biz.description ? `
X-WA-BIZ-NAME:${(conn.chats[njid]?.vname || name || await conn.getName(njid)).replace(/\n/, '\\n')}
