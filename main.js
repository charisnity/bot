...
const authState = await useMultiFileAuthState('.sessions')

const connectionOptions = {
  printQRInTerminal: true,
  auth: authState.state,
  version: [2, 2230, 15],
  browser: ['UnknownBot', 'Safari', '1.0.0']
  //logger: P({ level: 'info' })
}
...
function clearTmp() {
  ...
  return filename.map(file => {
    ...
    if (stats.isFile() && (Date.now() - stats.mtimeMs >= 1000 * 60 * 10)) return unlinkSync(file) // 10 minutes
    return false
  })
}
